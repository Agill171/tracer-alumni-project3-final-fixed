<x-layouts.app-tracer>
    ...
    <form method="GET" action="{{ route('alumni.index') }}"
        ...
        <div>
            ...
            <select id="status" name="status" class="w-full rounded-xl border border-slate-300 px-4 py-3">
        ...
        <div class="flex gap-2">
            <button type="submit" class="px-5 py-3 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-medium">
                Filter
            </button>
            @if(request()->hasAny(['q', 'status']))
                <a href="{{ route('alumni.index') }}" class="px-4 py-3 rounded-xl bg-slate-200 hover:bg-slate-300 font-medium">
                    Reset
                </a>
            @endif
        </div>
            ...
            @foreach($alumnis as $alumni)
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
                    <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                        <div class="space-y-2">
                            <h3 class="text-xl font-semibold text-slate-900">{{ $alumni->nama }}</h3>
                            <p><span class="font-medium">NIM:</span> {{ $alumni->nim ?: '-' }}</p>
                            <p><span class="font-medium">Program Studi:</span> {{ $alumni->prodi ?: '-' }}</p>
                            <p><span class="font-medium">Tempat Bekerja:</span> {{ $alumni->tempat_bekerja ?: '-' }}</p>
                            <p>
    ...
    @else
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 p-8 text-center">
            <p class="text-lg font-medium text-slate-700">Data alumni tidak ditemukan.</p>
            <p class="text-slate-500 mt-2">Ubah filter, tambahkan data alumni, atau lakukan import Excel.</p>
        </div>
    @endif
</x-layouts.app-tracer>
