<x-layouts.app-tracer>
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div>
            ...
            <p class="text-slate-600 mt-1">
                Profil target, pencarian manual, verifikasi identitas, dan data Project 4.
            </p>
        </div>
    ...
    <div class="mb-8 rounded-2xl border border-blue-200 bg-blue-50 p-5">
        <h3 class="font-semibold text-blue-900">
            Workflow Verifikasi Tanpa API
        </h3>

        <p class="text-sm text-blue-800 mt-2">
            1. Buka salah satu tautan pencarian.
            2. Cocokkan nama, kampus/prodi, timeline, dan bidang.
            3. Catat temuan beserta link bukti.
            4. Jika identitas cukup kuat, isi data Project 4 melalui tombol Edit Data Project 4.
        </p>
    </div>
    ...
    <div class="grid lg:grid-cols-2 gap-6 mb-8">
        <section class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            <dl class="grid grid-cols-[150px_1fr] gap-x-3 gap-y-2 text-sm md:text-base">
                ...
                <dt class="font-medium">NIM</dt>
                <dd>{{ $alumni->nim ?: '-' }}</dd>

                <dt class="font-medium">Program Studi</dt>
                <dd>{{ $alumni->prodi ?: '-' }}</dd>

                <dt class="font-medium">Angkatan</dt>
                <dd>{{ $alumni->angkatan ?: '-' }}</dd>

                <dt class="font-medium">Tahun Lulus</dt>
                <dd>{{ $alumni->tahun_lulus ?: '-' }}</dd>
        ...
        <section class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6">
            ...
            <dl class="space-y-2 text-sm md:text-base">
                <div>
                    <dt class="font-medium inline">Tempat Bekerja:</dt>
                    <dd class="inline">{{ $alumni->tempat_bekerja ?: '-' }}</dd>
                </div>

                <div>
                    <dt class="font-medium inline">Alamat:</dt>
                    <dd class="inline">{{ $alumni->alamat_bekerja ?: '-' }}</dd>
                </div>

                <div>
                    <dt class="font-medium inline">Posisi:</dt>
                    <dd class="inline">{{ $alumni->posisi ?: '-' }}</dd>
                </div>

                <div>
                    <dt class="font-medium inline">Kategori:</dt>
                    <dd class="inline">{{ $alumni->kategori_pekerjaan ?: '-' }}</dd>
                </div>
            </dl>
    ...
    <section class="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-8">
        <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5 mb-5">
            <div>
                ...
                <p class="text-slate-600 mt-1">
                    Sistem menyiapkan query berdasarkan identitas alumni.
                    Buka hasil pencarian di tab baru dan verifikasi kandidat secara manual.
                </p>
            </div>

            <a href="{{ route('pelacakan.create', $alumni) }}"
               class="shrink-0 px-4 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-medium">
                + Catat Hasil Verifikasi
            </a>
        </div>

        <form action="{{ route('pelacakan.query.store', $alumni) }}"
              method="POST"
              class="space-y-4">
            @csrf
            ...
            <button type="submit"
                    class="px-5 py-3 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-medium">
                Buat dan Simpan Tautan Pencarian
            </button>
        </form>

        <div class="mt-6 overflow-x-auto">
            <table class="w-full min-w-[860px] text-sm border-separate border-spacing-0">
                <thead>
                    <tr class="text-left bg-slate-100">
                        <th class="p-3 border-y border-l border-slate-200 rounded-l-xl">
                            Prioritas
                        </th>

                        <th class="p-3 border-y border-slate-200">
                            Sumber
                        </th>
                        ...
                        <th class="p-3 border-y border-r border-slate-200 rounded-r-xl">
                            Aksi
                        </th>
                    </tr>
                </thead>
                ...
                <tbody>
                    @forelse($alumni->queryPelacakans as $query)
                        <tr class="align-top">
                            ...
                            <td class="p-3 border-b border-slate-200 font-medium">
                                {{ data_get(
                            ...
                            <td class="p-3 border-b border-slate-200 max-w-xl break-words">
                                {{ $query->query }}
                            </td>

                            <td class="p-3 border-b border-slate-200">
                                {{ optional($query->generated_at)->format('d-m-Y H:i') ?: '-' }}
                            </td>

                            <td class="p-3 border-b border-slate-200">
                                <div class="flex flex-wrap gap-2">
                                    <a href="{{ $query->url_pencarian }}"
                                       target="_blank"
                                       rel="noopener noreferrer"
                                    ...
                                    <a href="{{ route('pelacakan.create', [
                                            'alumni' => $alumni,
                                            'query' => $query->query,
                                            'source' => $query->sumber,
                                        ]) }}"
                                       class="inline-block px-3 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-medium">
                                        Catat Temuan
                                    </a>
                                </div>
                            </td>
        ...
        <div class="flex items-center justify-between gap-4 mb-5">
            <div>
                ...
                <p class="text-slate-600 mt-1">
                    ...
                    confidence score, dan petugas pencatat.
                </p>
            </div>
        ...
        @forelse ($alumni->hasilPelacakans as $pelacakan)
            @php
                ...
                $scoreClass = match($pelacakan->kategori_kecocokan) {
                    \App\Models\HasilPelacakan::KATEGORI_KUAT =>
                        'bg-emerald-100 text-emerald-700',

                    \App\Models\HasilPelacakan::KATEGORI_VERIFIKASI =>
            ...
            <article class="border border-slate-200 rounded-2xl p-5 mb-4">
                <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    <div class="space-y-2">
                        <h4 class="text-lg font-semibold">
                            {{ $pelacakan->judul_temuan ?: 'Temuan dari '.$pelacakan->sumber_temuan }}
                        </h4>
                        ...
                        <p>
                            ...
                            {{ optional($pelacakan->tanggal_ditemukan)->format('d-m-Y') ?: '-' }}
                        </p>
                ...
                <div class="flex flex-wrap gap-2 mt-5">
                    ...
                    <form action="{{ route('pelacakan.destroy', $pelacakan) }}"
                          method="POST"
